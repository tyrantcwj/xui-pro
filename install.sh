...
need_root() { if [ "$(id -u)" -ne 0 ]; then echo "Please run as root." >&2; exit 1; fi; }

detect_arch() { case "$(uname -m)" in x86_64|amd64) echo "amd64" ;; aarch64|arm64) echo "arm64" ;; *) echo "Unsupported architecture: $(uname -m)" >&2; exit 1 ;; esac; }

download_url() { local arch="$1"; if [ "$VERSION" = "latest" ]; then echo "https://github.com/${REPO}/releases/latest/download/xui-pro-linux-${arch}.tar.gz"; else echo "https://github.com/${REPO}/releases/download/${VERSION}/xui-pro-linux-${arch}.tar.gz"; fi; }
source_url() { echo "https://codeload.github.com/${REPO}/tar.gz/refs/heads/main"; }
latest_release_tag() { curl -fsSL "https://api.github.com/repos/${REPO}/releases/latest" | sed -n 's/.*"tag_name"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' | head -n1; }
...
install_files() {
  ...
  tmp="$(mktemp -d)"
  trap "rm -rf '$tmp'" EXIT
  if [ "$FORCE_SOURCE" = "1" ]; then echo "Source build was requested. Building xui-pro from main..."; build_from_source "$arch" "$tmp"; return; fi
  echo "Downloading xui-pro ${VERSION} for linux-${arch}..."
  if ! curl -fL "$(download_url "$arch")" -o "$tmp/xui-pro.tar.gz"; then echo "Release asset was not found. Falling back to source build..."; build_from_source "$arch" "$tmp"; return; fi
  install_package "$tmp"
}
...
installed_version_label() {
  local label=""
  case "$MODE" in
    master) if [ -x "$INSTALL_DIR/xuid" ]; then label="$($INSTALL_DIR/xuid --version 2>/dev/null || true)"; fi ;;
    agent) if [ -x "$INSTALL_DIR/xui-agent" ]; then label="$($INSTALL_DIR/xui-agent --version 2>/dev/null || true)"; fi ;;
  esac
  if [ -z "$label" ]; then label="xui-pro ${MODE} ${VERSION}"; fi
  echo "$label"
}
...
install_package() {
  ...
  if [ -d "$tmp/reality" ]; then cp -R "$tmp/reality" "$INSTALL_DIR/"; fi
}

ensure_go() {
  local arch="$1" need_install=0
  if command -v go >/dev/null 2>&1; then local minor; minor="$(go version | sed -n 's/.*go[0-9][0-9]*\.\([0-9][0-9]*\).*/\1/p' | head -n1)"; if [ -z "$minor" ] || [ "$minor" -lt 22 ]; then need_install=1; fi; else need_install=1; fi
  if [ "$need_install" -eq 0 ]; then return; fi
  local go_arch="$arch" toolchain_dir="/usr/local/xui-pro-toolchain" go_tar="/tmp/go${GO_VERSION}.linux-${go_arch}.tar.gz"
...
build_from_source() {
  local arch="$1" tmp="$2" src="$tmp/src" commit build_version
...
service_active_or_enabled() { local service="$1"; systemctl is-active --quiet "$service" 2>/dev/null || systemctl is-enabled --quiet "$service" 2>/dev/null; }
env_has_value() { local file="$1" key="$2"; [ -f "$file" ] && grep -Eq "^${key}=.+" "$file"; }
assert_exclusive_mode() {
  if [ "$FORCE_MODE_SWITCH" = "1" ]; then return; fi
  case "$MODE" in
    master) if service_active_or_enabled "xui-pro-agent.service" || env_has_value "$CONFIG_DIR/agent.env" "XUI_MASTER" || env_has_value "$CONFIG_DIR/agent.env" "XUI_NODE_ENDPOINT" || env_has_value "$CONFIG_DIR/agent.env" "XUI_NODE_ID"; then echo "Refusing to install master: this VPS already looks like an Agent node." >&2; echo "Master and Agent are mutually exclusive on the same machine. Use XUI_PRO_FORCE=1 to switch." >&2; exit 1; fi ;;
    agent) if service_active_or_enabled "xui-pro.service" || env_has_value "$CONFIG_DIR/master.env" "XUI_LISTEN"; then echo "Refusing to install agent: this VPS already looks like the Master panel." >&2; echo "Master and Agent are mutually exclusive on the same machine. Use XUI_PRO_FORCE=1 to switch." >&2; exit 1; fi ;;
  esac
}

write_master_env() { cat > "$CONFIG_DIR/master.env" <<EOF
XUI_LISTEN=${XUI_LISTEN}
XUI_REALITY_LIBRARY=${INSTALL_DIR}/reality/domains.json
EOF
}
write_agent_env() {
  if [ -z "$XUI_NODE_ID" ]; then XUI_NODE_ID="$(hostname)"; fi
  if [ -z "$XUI_NODE_NAME" ]; then XUI_NODE_NAME="$XUI_NODE_ID"; fi
...
enable_mode() {
  ...
  case "$MODE" in
    master) write_master_env; systemctl enable --now xui-pro.service; write_installed_version; echo "XUI Pro master installed."; echo "Version: $(installed_version_label)"; echo "Check: xui-pro status" ;;
    agent) write_agent_env; systemctl enable --now xui-pro-agent.service; write_installed_version; echo "XUI Pro agent installed."; echo "Version: $(installed_version_label)"; echo "Check: xui-pro agent-status" ;;
    *) echo "Unknown mode: $MODE" >&2; usage; exit 1 ;;
  esac
}
main() { need_root; parse_args "$@"; if [ "$VERSION" = "latest" ] && [ "$FORCE_SOURCE" != "1" ]; then VERSION="$(latest_release_tag || true)"; VERSION="${VERSION:-latest}"; fi; assert_exclusive_mode; arch="$(detect_arch)"; install_files "$arch"; write_services; enable_mode; }
main "$@"
